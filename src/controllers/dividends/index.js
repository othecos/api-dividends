...
import { DIVIDENDS_CALENDAR_URL, DIVIDENDS_DICTIONARY, months } from "../../configs/dividends";
...
export default class DividendsController {
                        ...
                        rows.forEach((row, index) => {
                            try {
                                ...
                                if (word.key == 'value') {
                                    ...
                                    dividend = {
                                        ...dividend,
                                        ...this.formatStockValuesV2(word, value, nextValue)
                                    }
                                }
    ...
    formatStockValuesV2(word, firtColumn, secondColumn) {
        let field = {
            'value_v2':[]
        }
        let [firstColumnValue, firstColumnStockType] = firtColumn.match(/([0-9](,|),[0-9]+)|(PN|ON|UNIT)/gi)
        const firstPrice = {
            type: firstColumnStockType,
            price: parseFloat(firstColumnValue.replace(',', '.'))
        }
        field['value_v2'].push(firstPrice)
        if (secondColumn) {
            let [secondColumnValue, secondColumnStockType] = secondColumn.match(/([0-9],[0-9]+)|(PN|ON|UNIT)/gi)
            const secondPrice =  {
                type: secondColumnStockType,
                price: parseFloat(secondColumnValue.replace(',', '.'))
            }
            field['value_v2'].push(secondPrice)
        } 
        return field
    }
